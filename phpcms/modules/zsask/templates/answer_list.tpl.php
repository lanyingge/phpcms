...
if(is_array($infos)){
	...
	<tr>
		...
		<td><a href="<?php if (empty($info['url'])) {echo APP_PATH.'index.php?m=zsask&a=answer&qid='.$info['qid'];}else{echo $info['url'];}?>" target="_blank"><?php echo $info['question']?></a> 
		 [<?php if ($info['aid']) {echo '<span style="color:red">已解决</span>';}else {echo '<span style="color:#999">'.format_date(get_lastanswer($info['qid'])).'</span>';}?>]
		</td>
...
<input name="dosubmit" type="submit" class="button" onclick="myform.submit();"
	...
	onclick="myform.action='?m=zsask&c=zsask&a=create_html&dosubmit=1';checkuid()" value="批量生成HTML">&nbsp;&nbsp;
