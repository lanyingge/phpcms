...
<form name="myform" id="myform" action="?m=ask&c=category&a=edit&menuid=<?php echo $_GET['menuid'] ?>" method="post">
<div class="pad-10">
<div class="col-tab">

<ul class="tabBut cu-li">
<li id="tab_setting_1" class="on" onclick="SwapTab('setting','on','',6,1);"><?php echo L('catgory_basic');?></li>
<li id="tab_setting_2" onclick="SwapTab('setting','on','',6,2);" style="display:none"><?php echo L('catgory_createhtml');?></li>
<li id="tab_setting_3" onclick="SwapTab('setting','on','',6,3);" style="display:none"><?php echo L('catgory_template');?></li>
<li id="tab_setting_4" onclick="SwapTab('setting','on','',6,4);"><?php echo L('catgory_seo');?></li>
<li id="tab_setting_5" onclick="SwapTab('setting','on','',6,5);" style="display:none"><?php echo L('catgory_private');?></li>
<li id="tab_setting_6" onclick="SwapTab('setting','on','',6,6);" style="display:none"><?php echo L('catgory_readpoint');?></li>
</ul>
...
<table width="100%" class="table_form ">
      <tr>
        ...
        <td>
		<?php echo form::select_category('category_ask_'.$this->siteid,$parentid,'name="info[parentid]"',L('please_select_parent_category'),0,-1);?> 
		</td>
      </tr>
      <tr>
	...
	<tr>
        ...
        <td><?php echo form::images('info[image]', 'image', $image, 'ask');?></td>
      </tr>
	<tr>
...
<table width="100%" class="table_form">
	...
	<?php
		echo form::urlrule('ask','category',0,$setting['category_ruleid'],'name="category_php_ruleid"');
	?>
	</div>
	<div id="category_html_ruleid" style="display:<?php if(!$setting['ishtml']) echo 'none';?>">
	<?php
		echo form::urlrule('ask','category',1,$setting['category_ruleid'],'name="category_html_ruleid"');
	?>
	</div>
	</td>
    </tr>
	...
	<tr>
	  ...
	  <?php
		echo form::urlrule('ask','show',0,$setting['show_ruleid'],'name="show_php_ruleid"');
	?>
	</div>
	<div id="show_html_ruleid" style="display:<?php if(!$setting['content_ishtml']) echo 'none';?>">
	  <?php	
		echo form::urlrule('ask','show',1,$setting['show_ruleid'],'name="show_html_ruleid"');
	?>
	</div>
	</td>
    </tr>
...
<!--
	...
	function load_file_list(id) {
		if(id=='') return false;
		$.getJSON('?m=ask&c=category&a=public_tpl_file_list&style='+id+'&catid=<?php echo $catid?>', function(data){$('#category_template').html(data.category_template);$('#list_template').html(data.list_template);$('#show_template').html(data.show_template);});
	}
